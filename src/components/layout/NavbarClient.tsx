                ...
                {isMobileMenuOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed inset-y-0 right-0 w-full max-w-xs bg-white shadow-2xl z-50 md:hidden flex flex-col"
                        >
                            ...
                            <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
                                {navLinks.map((link) => (
                                    <Link
                                        ...
                                        href={link.href}
                                        onClick={() => setIsMobileMenuOpen(false)}
                                        className="block px-4 py-3 text-lg font-medium text-gray-600 hover:text-pink-600 hover:bg-pink-50 rounded-xl transition-all"
                                    >
                                        {link.label}
                                    </Link>
                                ))}
                                ...
                                <div className="pt-6 mt-6 border-t border-gray-100">
                                    {session ? (
                                        <Link
                                            href="/admin"
                                            onClick={() => setIsMobileMenuOpen(false)}
                                            className="flex items-center justify-center gap-2 w-full px-6 py-3 rounded-xl bg-gray-900 text-white font-medium hover:bg-gray-800 transition-all"
                                        >
                                        ...
                                        <Link
                                            href="/login"
                                            onClick={() => setIsMobileMenuOpen(false)}
                                            className="flex items-center justify-center gap-2 w-full px-6 py-3 rounded-xl bg-pink-600 text-white font-medium hover:bg-pink-700 transition-all"
                                        >
